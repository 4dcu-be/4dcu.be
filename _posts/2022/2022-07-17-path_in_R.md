...
description: "A quick snippet using the this.path package in R to set the working directory to a script's own location, so code runs portably across machines and RStudio."
...
those that like to open their .R files in [RStudio] without creating a proper project (like I used to, now I do make an RProject with its own renv for everything) are likely to run into 
