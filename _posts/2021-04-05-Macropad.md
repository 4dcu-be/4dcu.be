...
while True:
    ...
    for ix, led in enumerate(leds):
        ...
        duty_cycles[ix] = max(duty_cycles[ix] - 600, 0)

    time.sleep(0.001)
```

When using this code, the LED under each button will light up as soon as you press a button and then quickly fade out. 
The fade out speed can be changed by changing 600 in ```duty_cycles[ix] = max(duty_cycles[ix] - 600, 0)``` to a higher
...
Once you are sure everything works, put everything in the case carefully. Soldered joints can be fragile to be gentle with
...
## Links
  ...
  * [My Case on Thingiverse](https://www.thingiverse.com/thing:4816077) a remixed from:
