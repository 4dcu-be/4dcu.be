...
byline: ""
description: "Een genetisch kunstalgoritme van Python vertalen naar Rust met Claude Code als voornaamste codeerhulp, en onderzoeken hoe agentic AI werkt wanneer je een absolute beginner bent in een taal."
...
Dit staat duidelijk ver van Python-code af: we moeten het type van de getallen opgeven, expliciet bepalen welke variabelen muteerbaar zijn enzovoort. Claude Code deed het echter erg goed door aan te duiden waar bepaalde Rust-paradigma's van belang zijn. Ook de manier waarop tests in het geteste bestand zelf worden geïmplementeerd, verschilt sterk van Python. Hier stootte ik op het eerste nadeel van agentic coding: dit is geen goede manier om de syntaxis van een nieuwe taal te leren, althans niet voor mij. Om een nieuwe syntaxis te onthouden, moet ik het op de moeilijke manier doen en een paar duizend regels code uittypen.
...
De fitnessfunctie is waar Claude Code het interessant maakte: het implementeerde die als een parallelle functie die verschillende pixels in verschillende threads berekent. De functie `.par_iter()` uit de rayon-library maakt dit opnieuw multithreaded, zodat alle beschikbare cores in je machine tijdens deze stap worden benut.
...
Omdat ik Claude Code de opdracht had gegeven om de app in fases te bouwen en bij elke stap tests te draaien, compileerde de code na een paar rondes bouwen, falen, debuggen en opnieuw proberen, en was de app klaar om door mij te worden getest. Op een klein probleempje bij het renderen van de afbeelding na werkte alles!

Ik testte de app met dezelfde afbeelding die ik in 2020 gebruikte, Van Goghs [*De sterrennacht*](https://en.wikipedia.org/wiki/The_Starry_Night), en het eerste wat me opviel, was dat mijn CPU-belasting snel naar 95% ging en daar bleef!
...
"Rewrite in Rust" is misschien een beetje een meme, maar ik merkte meteen een versnelling toen ik mijn code draaide. Om beter te begrijpen wat er aan de hand was, deed ik een paar testruns en vergeleek ik de looptijden met die van de oorspronkelijke Python-implementatie. Elke test gebruikte een doelafbeelding van 570×452 pixels; de andere parameters staan in de tabellen hieronder.

De resultaten waren wat verrassend. Op één thread was Rust eigenlijk trager dan Python, waarschijnlijk omdat de Pillow-library in Python het grootste deel van het tekenen en vergelijken van afbeeldingen efficiënter afhandelt dan de initiële implementatie. Het bracht ook iets aan het licht wat ik me niet had gerealiseerd: Python schaalde bijzonder slecht over meerdere cores en werd merkbaar trager (!) naarmate er meer threads werden gebruikt. Rust daarentegen ging prachtig om met multithreading. Op mijn 8-core Ryzen 7 3700X was de parallelle efficiëntie uitstekend, en zodra Rust meerdere cores mocht gebruiken, liet het Python moeiteloos achter zich.
...
Die extra optimalisaties bleken bijzonder de moeite waard: in deze kleine test ging de looptijd na de verbetering van de tekenroutine van 127 naar 23 seconden, en met SIMD zakte hij verder tot amper 11 seconden. Dat is een vertienvoudiging tegenover de niet-geoptimaliseerde Rust-versie, en met alle cores benut zijn we nu 52 keer zo snel als de Python-versie ooit kan draaien.

Houd er wel rekening mee dat deze benchmarks uit slechts enkele runs bestaan, dus beschouw de cijfers als indicatief en niet als absoluut. Het verschil in performantie tussen de implementaties is echter meteen duidelijk.
...
De echte waarde zat dus niet alleen in de 50 keer zo hoge snelheid tegenover Python, hoe fijn die ook is (en die laat vergelijkingen toe met meer vormen, grotere populaties en meer generaties, zoals je hierboven ziet bij Grant Woods [*American Gothic*](https://en.wikipedia.org/wiki/American_Gothic), waarvoor 400 driehoeken of 600 cirkels werden gebruikt). Ze zat in de mogelijkheid om met beperkte tijd een nieuwe taal te verkennen en snel een indruk te krijgen van de tooling en het ecosysteem. Claude Code inzetten voor een project dat ik zelf niet had kunnen uitvoeren, was ook een nederig makende ervaring: deze tools zijn ongelooflijk krachtig! Maar het zijn geen volledig autonome agents die zo'n project van A tot Z zonder sturing aankunnen. De sleutel is de AI behandelen als een ervaren pair programmer: hij kan de code schrijven, maar jij moet nog steeds de architectuur begrijpen, de implementatie nakijken en verifiëren dat ze het juiste probleem oplost. Om nieuwe technologieën te verkennen en tegelijk van een concreet, echt project te leren, werkte deze aanpak opmerkelijk goed.
