...
description: "Adding Flesh and Blood support to DeckLock, a Pelican-based static site for showcasing card game decks, pulling card data from the FaBDB.net API."
...
> **Update (November 2025):** [FaBDB.net] has since been discontinued and is no longer online (the link now
> points to an [archived snapshot]). As a result [DeckLock] no longer relies on its API — Flesh and Blood
> decklists are now maintained as manually written `.fab` text files instead. The walkthrough below is kept
> for historical context, but the `fabdb.net` links no longer work.
...
[archived snapshot]: https://web.archive.org/web/20220105152741/https://fabdb.net/
