...
description: "Adding Gwent support to the DeckLock Pelican generator, handling version-specific cards by pulling data from The Voice of Gwent instead of the official site."
...
cards themselves can't be changed. Occasionally, an erratum is issued or cards that are too powerful are banned
...
with a `.gwent` extension in the `content` folder and the Gwent [Pelican] plugin included in the new version of [DeckLock]
