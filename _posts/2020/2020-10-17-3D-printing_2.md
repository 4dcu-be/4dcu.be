...
description: "Five practical reasons to own a 3D printer, from electronics enclosures and custom storage to brackets, showing real projects printed on a Creality CR10S."
...
While other projects are still brewing, I wanted to highlight five cases where 3D printing has helped out projects of
...
Electronics projects often start with a handful of printed circuit boards (PCB) that are soldered together and have some
...
work to house your project ... for which I lack the tools and skill ... So having access to a 3D printer has been a
boon. The Zigbee stick I purchased to connect Zigbee devices to Home Assistant 
(check [this post]({% post_url 2020/2020-09-10-MQTT %})) is a great example of how a bare PCB can be turned into a good
...
next to the router. A 3D printed case here costs a few dollars for 100-200 grams of filament and you can pick one that
fits your needs exactly. I mostly needed something which could be wall mounted easily, though you might want something
...
Not long before this blog started, I built a GameBoy Zero, which requires retrofitting a new PCB, a Raspberry Pi Zero
...
have seen in other people's houses is to cut up a pool noodle and put that over the door handle ... which I would
