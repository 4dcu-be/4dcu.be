...
description: "Building DeckLock, a Python Pelican static site generator to track card game collections, starting with KeyForge decks fetched from their unique identifiers."
...
This is perfect for our application, we can get all information (name, decklist, ...) of a scanned deck directly from the
[KeyForge] website. The only thing we need is the deck's unique identifier, everything else we'll fetch automatically.
Furthermore, using the same identifier additional statistics on the deck (like the estimated quality of the cards, 
synergies between them, ...) can be obtained from [Decks of KeyForge].
...
As we are using a virtual environment with pelican installed there, the path to the executable in
...
[https://decksofkeyforge.com/](https://decksofkeyforge.com/). Create a file `.env` and add the line below.
...
Now, add a `keyforge.json` file to `./content/data`, structured as follows with the identifiers of the decks to include.
A file with the KeyForge Decks I own is included as an example, the structure should be as shown below.
...
**Writer** that will combine data and template into a single html file. Generators are used to generate overview pages
