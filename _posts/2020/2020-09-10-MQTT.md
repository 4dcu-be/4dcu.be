...
description: "Connecting an Aqara Cube to Home Assistant without a hub by flashing a Zigbee dongle with Z-Stack firmware and using Zigbee2MQTT on a Raspberry Pi."
...
one ... I decided to do the latter and expand my Raspberry Pi which runs Home Assistant with a Zigbee antenna.
...
Since the Zigbee dongle is a bare PCB, I 3D printed a case designed by hansaya for this type of dongle which can be
...
CC2531 chip. There are antennas available with this firmware pre-loaded, which cost only a few bucks more. So,
...
connected to the debug pins on the Zigbee Antenna. There is plenty of information in the [Zigbee2MQTT documentation] on how
...
Now everything works, the 3D printed case can be snapped on the Zigbee stick and we can move on to the final phase.
...
broker it should be listening to, to do so we'll add these lines to `configuration.yaml`.
...
playing around with different options and lots of googling, but persistence persevered and the following automations
worked.
