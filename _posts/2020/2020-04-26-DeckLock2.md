...
description: "Expanding the DeckLock Pelican generator to support Magic the Gathering decks, parsing the Magic Workstation format and fetching card data and images online."
...
are used to store the name of the deck, who created it, ... other lines contain the number of times a card is included in
...
Based on the name of the card, all other information can be pulled from [ScryFall], a website that has images for all
...
Below you'll find the code for the Reader, this needs to be a class that is built upon [Pelican]'s BaseReader, it needs
...
The function `read` attached to the MTGReader is required, here the metadata is set. A category and date are required by
...
Unlike in our previous post, the overview page for Magic makes use of [Pelican]'s built-in functions. However, we do need
...
might not be a smart thing to do. Most likely it even is illegal. To this end, there are a few options included in the
file `publishconf.py` which is used when building the version intended for on-line use. The latter can be built with
...
for both games by setting `STATIC_EXCLUDES`, we prevent [Pelican] from copying the downloaded images to the output intended
