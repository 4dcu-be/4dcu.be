...
tags:	python pelican blog games keyforge
...
Pelican uses three components to create pages, a Reader (which reads files and converts them), a generator that takes
the Reader's data, processes it, generates a url and sends it with the correct template to the writer that will combine
data and template into a single html file.

The function *get_keyforge_external_data* will write everything required to a single json file, so a Reader class can
be omitted here, but we'll have to create a generate that loads the data, builds a url for each deck and ships the 
relevant data to our templates. 
...
Support for more games, like [Magic: the Gathering] and [Gwent], will be implemented in a next post. Stay tuned !
